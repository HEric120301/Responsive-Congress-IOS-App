...
class CommJointController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request(url, method: .post, parameters: parameters).responseJSON { response in
            ...
            if let resData = swiftjsondata["results"].arrayObject {
                ...
                self.arr = self.arr.filter({($0["chamber"] as! String) == "joint"})
                self.arr.sort{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "CommJointCell", for: indexPath) as! CommJointCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "CommJointDetail", sender: indexPath)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let destinationDT = segue.destination as? CommJointDetail {
...
//search
extension CommJointController: UISearchResultsUpdating {
    @available(iOS 8.0, *)
